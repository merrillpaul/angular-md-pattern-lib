...
gulp.task("ngc", function() {
	//let stat =  ngc( ['-p', `${tmpFolder}/tsconfig.es5.json`]);
	//if (stat === 1) {
	...
	return (gulp
			...
			.pipe(
				rollup({
					...
					input: `${buildFolder}/public-api.js`,
...
gulp.task("rollup:umd", function() {
	return (gulp
			...
			.pipe(rename("pearson-angular-material.umd.js"))
			.pipe(gulp.dest(distFolder)) );
});
...
/**
 * 7. Correct the /index that causes due to https://github.com/angular/angular/issues/17171#issuecomment-315264322
 */
gulp.task("correct:import", function() {
	return gulp
		.src([`${distFolder}/**/*.js`])
		.pipe(replace(/hammerjs\/index/g, "hammerjs"))
		.pipe(replace(/iscroll\/index/g, "iscroll"))
		.pipe(replace(/@angular\/flex-layout\/index/g, "@angular/flex-layout"))
		.pipe(replace(/@swimlane\/ngx-charts\/index/g, "@swimlane/ngx-charts"))
		.pipe(gulp.dest(distFolder));
});
