...
export class MediaDirective implements OnInit, OnDestroy {
    ...
    set query(query: string) {
        if (!query) {
            throw new Error('Query needed for [psnIfMedia] directive.');
        }
        this._query = query;
    }
    ...
    constructor(private _renderer: Renderer2,
        ...
        private _mediaService: MediaQueryService) { }

    ngOnInit(): void {
        this._mediaChange(this._mediaService.query(this._query));
        this._subscription = this._mediaService.observeFor(this._query).subscribe((matches: boolean) => {
